    ...
    <div class="main-content">
      ...
      <div class="processing-history">
        ...
        <el-table :data="processingHistory" style="width: 100%">
          <el-table-column prop="filename" label="文件名" width="1"></el-table-column>
          <el-table-column prop="type" label="文件类型" width="1">
            <template #default="scope">
              <el-tag :type="getTypeColor(scope.row.type)">{{ scope.row.type }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="size" label="文件大小" width="1"></el-table-column>
          <el-table-column prop="status" label="处理状态" width="1">
            <template #default="scope">
              <el-tag :type="getStatusColor(scope.row.status)">{{ scope.row.status }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="extractedText" label="提取文字预览" width="1">
            <template #default="scope">
              <div class="text-preview">{{ scope.row.extractedText }}</div>
            </template>
          </el-table-column>
          <el-table-column prop="uploadTime" label="上传时间" width="1"></el-table-column>
